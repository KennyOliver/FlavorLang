...
Token *tokenize(const char *source) {
  ...
  while (pos < length) {
    ...
    if (isdigit(c)) {
      handle_number(source, &pos, length, &tokens, &token_count, &capacity,
                    line);
      continue;
    }

    if (c == '"') {
      handle_string(source, &pos, length, &tokens, &token_count, &capacity,
                    line);
      continue;
    }

    if (is_valid_identifier_start(c)) {
      handle_identifier_or_keyword(source, &pos, length, &tokens, &token_count,
                                   &capacity, line);
      continue;
    }

    if (strchr("=+-*/<>", c)) {
      handle_operator(source, &pos, length, &tokens, &token_count, &capacity,
                      line);
      continue;
    }
